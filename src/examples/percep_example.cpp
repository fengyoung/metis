...
void PrintHelp()
{
	cout<<"Usage: percep_example [--train <config_file> <training_patterns_file> <out_model_file>]"<<endl;   
...
{
	cout<<"== Perceptron Example: Increacing Training =="<<endl; 
	Timer timer;
	...
	if(perceptron.Load(sPrevModelFile) != _METIS_NN_SUCCESS)
	{
		cout<<"failed to initialize the Perceptron from previous model file "<<sPrevModelFile<<endl; 
		return; 
	}

	perceptron.PrintParams(cout); 

	perceptron.Train(vtr_patts); 

	if(perceptron.Save(sModelFile) != _METIS_NN_SUCCESS)
...
{
	cout<<"== Perceptron Example: Training =="<<endl; 
	Timer timer;
	...
	Perceptron perceptron; 
	if(!perceptron.InitFromConfig(sConfigFile, vtr_patts[0]->m_nXCnt, vtr_patts[0]->m_nYCnt))
	{
