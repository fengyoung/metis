...
void TrainDemo(const char* sConfigFile, const char* sPattFile, const char* sModelFile)
{
	...
	TypeDefs::Print_PerceptronLearningParamsT(cout, mlp_nn.GetLearningParams()); 
	cout<<"--"<<endl; 
	TypeDefs::Print_MLPParamsT(cout, mlp_nn.GetArchParams()); 
	cout<<"==========================="<<endl; 

	mlp_nn.TrainAndValidate(vtr_patts); 
