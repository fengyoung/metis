...
void PrintHelp()
{
	cout<<"Usage: fmsnn_example [--train <config_file> <training_patterns_file> <out_model_file>]"<<endl;   
...
void TrainDemo(const char* sConfigFile, const char* sPattFile, const char* sModelFile)
{
	...
	{
		cout<<"failed to initialize the FMS_NN from config file "<<sConfigFile<<endl; 
		return; 
	}

	TypeDefs::Print_PerceptronLearningParamsT(cout, fms_nn.GetLearningParams()); 
	cout<<"--"<<endl; 
	TypeDefs::Print_FMSNNParamsT(cout, fms_nn.GetArchParams()); 
	cout<<"==========================="<<endl; 

	fms_nn.TrainAndValidate(vtr_patts); 
	if(fms_nn.Save(sModelFile) != _METIS_NN_SUCCESS)
...
{
	cout<<"== FMSNN Example: Testing =="<<endl; 
	
	FMSNN fms_nn;
	int32_t ret = fms_nn.Load(sModelFile); 
	if(ret != _METIS_NN_SUCCESS)
	{
		cout<<"failed to load the FMS_NN model from "<<sModelFile<<endl; 
	...
	{
		ret = fms_nn.Predict(y, y_len, vtr_patts[i]->m_x, vtr_patts[i]->m_nXCnt); 
		if(ret == _METIS_NN_SUCCESS)
		{
			success += 1;  
