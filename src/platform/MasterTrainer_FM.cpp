...
MasterTrainer_FM::MasterTrainer_FM() : MasterTrainer()
{
}


MasterTrainer_FM::~MasterTrainer_FM()
{
}


bool MasterTrainer_FM::InitByLearningConfig(const char* sLearningConfFile)
{
	if(m_nInput < 0 || m_nOutput < 0)
		return false; 
	return m_fm.InitFromConfig(sLearningConfFile, m_nInput, m_nOutput); 
}


bool MasterTrainer_FM::InitByModel(const char* sModelFile)
{
	if(m_fm.Load(sModelFile) != _METIS_NN_SUCCESS)
		return false;
	if(m_nInput > 0 && m_nOutput > 0)
	{
		if(m_fm.GetArchParams().input - 1 != m_nInput || m_fm.GetArchParams().output != m_nOutput)
			return false; 
	}
	return true; 
}
...
// 执行训练
void MasterTrainer_FM::Train(const char* sOutModelFile)
{
	printf("** FM Parallel Training **\n"); 

	FMLearningParamsT learning_params_t = m_fm.GetLearningParams(); 
	...
	printf("--\n");
	TypeDefs::Print_FMParamsT(cout, m_fm.GetArchParams()); 
	printf("=====\n");
	...
	while(iter < learning_params_t.iterations)
	{
		...
		// 将模型保存到临时文件中
		if(m_fm.Save(str_tmp_file.c_str()) != _METIS_NN_SUCCESS)
		{
...
// 从所有slave上收集更新后的模型数据
bool MasterTrainer_FM::CollectModel()
{
	string str_model;
	double avg_error; 
	FM fm; 
	...
	for(size_t i = 0; i < m_vtrSlaves.size(); i++) 
	{
		if(!GetModelFromSlave(m_vtrSlaves[i].c_str(), str_model, avg_error))
			return false; 
		if(!fm.SetByModelString(str_model.c_str()))
			return false;
		if(i == 0)
		{
			if(!m_fm.CombineWith(fm, 0.0, 1.0 / (double)m_vtrSlaves.size()))
				return false; 
			m_dAvgError = avg_error; 
		}
		else	
		{
			if(!m_fm.CombineWith(fm, 1.0, 1.0 / (double)m_vtrSlaves.size()))
				return false; 	
			m_dAvgError += avg_error; 
		}
	}
...
string MasterTrainer_FM::GetModelAsString()
{
	return m_fm.ConvToModelString(); 
}
