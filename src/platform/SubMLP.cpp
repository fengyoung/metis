...
SubMLP::SubMLP() : MLP(), m_bUpdateCancel(false)
{
}
...
double SubMLP::OnceUpdate(vector<Pattern*>& vtrPatts, int32_t& nOff, const int32_t nBatchCnt, 
...
double SubMLP::LoopUpdate(vector<Pattern*>& vtrPatts, const int32_t nBatchCnt, 
		const double dLearningRate, const ERegula eRegula)
{
	m_bUpdateCancel = false; 
	// create assistant variables for training
