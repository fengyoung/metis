...
SubFM::SubFM() : FM(), m_bUpdateCancel(false)
{
}
...
double SubFM::OnceUpdate(vector<Pattern*>& vtrPatts, int32_t& nOff, const int32_t nBatchCnt, 
...
double SubFM::LoopUpdate(vector<Pattern*>& vtrPatts, const int32_t nBatchCnt, 
		const double dLearningRate, const ERegula eRegula)
{
	m_bUpdateCancel = false; 
	// create assistant variables for training
