...
string ServConf::LogFile(const bool bIsMaster)
{
	char stmp[1024];
	if(bIsMaster)
		sprintf(stmp, "%s/master_%s_%d.log", m_strLogPath.c_str(), m_strHost.c_str(), m_nPort); 
	else	
		sprintf(stmp, "%s/slave_%s_%d.log", m_strLogPath.c_str(), m_strHost.c_str(), m_nPort); 
	return string(stmp); 
}
